...
class Sfm():
    ...
    def to_ply(self, path, point_cloud, colors) -> None:
        ...
        ply_header = '''ply
            ...
            end_header
            '''
        with open('E:' + '\\done\\' + self.img_obj.image_list[0].split('\\')[-2] + '.ply', 'w') as f:
    ...
    def __call__(self, enable_bundle_adjustment:boolean=False):
        ...
        for i in tqdm(range(total_images)):
            ...
            #plt.imshow(self.img_obj.image_list[0].split('\\')[-2], image_2)
        ...
        self.to_ply('E:\\done', total_points, total_colors)
        #self.to_ply(self.img_obj.path, total_points, total_colors)
        print("Completed Exiting ...")
        np.savetxt('E:\\done' + '\\done\\' + self.img_obj.image_list[0].split('\\')[-2]+'_pose_array.csv', pose_array, delimiter = '\n')
        #np.savetxt(self.img_obj.path + '\\res\\' + self.img_obj.image_list[0].split('\\')[-2]+'_pose_array.csv', pose_array, delimiter = '\n')

if __name__ == '__main__':
    #sfm = Sfm("Datasets\\manhole")
    sfm = Sfm("E:\\castle-P19")
    sfm()
#D:\\SOLINAS DOWNLOADS\\3d Modelling\\Multiview-3D-Reconstruction\\Datasets\\Herz-Jesus-P8
#E:\castle-P19
#E:\\done